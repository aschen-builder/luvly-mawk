...
namespace Klime.Scope
{
    ...
    public class Scope : MySessionComponentBase
    {
        ...
        private void SpawnScope(MatrixD inc_matrix)
        {
            ...
            {
                MyVisualScriptLogicProvider.SpawnPrefab("Scope", empt_pos, inc_matrix.Forward, inc_matrix.Up);
            }
            else
            {
                MyVisualScriptLogicProvider.SpawnPrefab("Scope", base_pos, inc_matrix.Forward, inc_matrix.Up);
            }
        }
                ...
                if (scope_grid != null && scope_block != null && !scope_grid.MarkedForClose)
                {
                    if (MyAPIGateway.Session.Player.Character != null)
                    {
                        if (MyAPIGateway.Session.Player.Character.EquippedTool is IMyAutomaticRifleGun)
                        {
                            ...
                            if (rifle.DefinitionId.SubtypeName.Contains("(Scope)"))
                            {
                                if (in_scope)
                                {
